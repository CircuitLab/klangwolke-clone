...
SoftwareSerial LCD = SoftwareSerial(RX_PIN, TX_PIN);
...
void setup()
{
  ...
  // for displaying current command
  for (int i = 0; i < 5; ++i) {
    pinMode(4 + i, OUTPUT);
  }
...
void loop()
{
  ...
  if (xbee.getResponse().isAvailable()) {
    if (xbee.getResponse().getApiId() == ZB_RX_RESPONSE) {
      ...
      displayLCD(startBytesInReceivedData[0] + " " + startBytesInReceivedData[1]);
  ...
  displayLCD("command: " + currentCommand);
}
...
void displayCurrentCommand()
{
  ...
  int idx = String("abcde").indexOf(currentCommand);
  if (0 <= idx) {
